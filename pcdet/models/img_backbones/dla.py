...
class DLASeg(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # # # restoration network ###########################
        # y_recon = self.latent(x1[5])
        # # y_recon = self.up4_3(y[-1])
        # y_recon = self.upsample_3(y_recon)
        # y_recon = torch.cat([y_recon, x1[4]], 1)
        # y_recon = self.reduce_chan_level3(y_recon)
        # y_recon = self.decoder_level3(y_recon)
        # data_dict['recon_feat3'] = y_recon
        # # y_recon = self.up3_2(y_recon) 
        # y_recon = self.upsample_3(y_recon)
        # y_recon = torch.cat([y_recon, x1[3]], 1)
        # y_recon = self.reduce_chan_level2(y_recon)
        # y_recon = self.decoder_level2(y_recon)
        # data_dict['recon_feat2'] = y_recon
        # # y_recon = self.up2_1(y_recon)
        # y_recon = self.upsample_3(y_recon)
        # y_recon = torch.cat([y_recon, x1[2]], 1)
        # y_recon = self.reduce_chan_level1(y_recon)
        # y_recon = self.decoder_level1(y_recon)
        # y_recon = self.refinement(y_recon)
        # data_dict['recon_feat1'] = y_recon
        # y_recon = self.output(y_recon)
        # data_dict['recon_image'] = y_recon
        # # # restoration network ###########################
               
        x = self.dla_up(x1) # x
        data_dict['skip_feature'] = x[:4] # addition
        y = []
