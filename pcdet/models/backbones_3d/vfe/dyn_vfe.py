...
class DynVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        sampled_coords, inverse_indices = coords.unique(sorted=False, return_inverse=True, dim=0) # 중복값 제거
        ...
        # SP fusion
        target_gt_points = batch_dict['target_gt_points'] # ~~ , 5
        target_gt_pc_range = target_gt_points.new_tensor(self.point_cloud_range)
        target_gt_voxel_size = target_gt_points.new_tensor(self.voxel_size)
        target_gt_keep, target_gt_coords = common_utils.get_in_range_mask(target_gt_points, target_gt_pc_range, target_gt_voxel_size, self.grid_size)
        target_gt_points, target_gt_coords = target_gt_points[target_gt_keep], target_gt_coords[target_gt_keep]
        target_gt_coords = torch.cat([target_gt_points[:, 0:1].long(), torch.flip(target_gt_coords, dims=[-1])], dim=-1)  # [bs_idx, Z, Y, X]
        target_gt_sampled_coords, target_gt_inverse_indices = target_gt_coords.unique(sorted=False, return_inverse=True, dim=0)
        batch_dict['target_gt_coords'] = target_gt_sampled_coords
