...
from .BasicBlock import Inception, Dilated_Pyramid
import numpy as np
...
class MyNet(ME.MinkowskiNetwork):
    ...
    def get_target_by_sp_tensor(self, out, coords_T):
          with torch.no_grad():
              ...
              target = np.in1d(out_sp_tensor_coords_1d, target_coords_1d.cpu())
    ...
    def forward(self, x, coords_T, device, prune=True):
        ...
        out = MEF.relu(out_s1)          # 32
        
        out_s2 = self.conv2(out)
        out_s2 = self.norm2(out_s2)
        out_s2 = self.block2(out_s2)
        out = MEF.relu(out_s2)          # 64
        
        out_s4 = self.conv3(out)
        out_s4 = self.norm3(out_s4)
        out_s4 = self.block3(out_s4)
        out = MEF.relu(out_s4)          # 128
        
        out_s8 = self.conv4(out)
        out_s8 = self.norm4(out_s8)
        out_s8 = self.block4(out_s8)
        out = MEF.relu(out_s8)          # 256
        
        out = self.conv4_tr(out)
        out = self.norm4_tr(out)
        out = self.block4_tr(out)
        out_s4_tr = MEF.relu(out)       # 128
        ...
        out_s1_tr = MEF.relu(out)       # 32 
        
        out = ME.cat(out_s1_tr, out_s1)
        out = self.conv1_tr(out)
        out = self.norm1_tr(out) ######
        out = self.block1_tr(out) #######
        out_s0 = MEF.relu(out)             # 32
        
        out_cls = self.final(out_s0)
        ...
        return out_cls, out_s0, out_s1_tr #, out_s2_tr #, out_cls, target, keep
