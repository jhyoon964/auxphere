...
def train(model, train_dataloader, test_dataloader, logger, writer, args, device):
  ...
  for i in range(start_step, args.global_step+1):
    ...
    # Forward.
    _, out_cls, target, keep = model(x, coords_T=coords_T, device=device, prune=False)
    loss = crit(out_cls.F.squeeze().requires_grad_(True), target.type(out_cls.F.dtype).requires_grad_(True).to(device))
    metrics = get_metrics(keep, target)
    print('-----------loss-------------', loss)
    # print('out_cls.F.squeeze()', out_cls.F.squeeze().shape)
    # print('out_cls.F.squeeze()', (out_cls.F.squeeze()))
    # print('-----------------target.type(out_cls.F.dtype)', (target.type(out_cls.F.dtype)))
    # print('target.type(out_cls.F.dtype)', target.type(out_cls.F.dtype).ndim)
    if torch.isnan(loss) or torch.isinf(loss):
        logger.info(f'\n== loss is nan ==, Step: {i}\n')
        continue
