...
class KittiDataset(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        lidar_file = self.root_split_path / 'velodyne_4_inter_refine_rain' / ('%s.bin' % idx)
        return self.client.load_to_numpy(str(lidar_file), dtype=np.float32).reshape(-1, 4)

    # SP fusion
    def get_target_gt_lidar(self, idx):
        gt_lidar_file = self.root_split_path / 'velodyne_gt' / ('%s.bin' % idx)
        return self.client.load_to_numpy(str(gt_lidar_file), dtype=np.float32).reshape(-1, 4)
    ...
    def get_gt_image(self, idx):
        img_file = self.root_split_path / 'image_gt' / ('%s.png' % idx)
        assert self.client.exists(img_file)
        return self.client.load_img(str(img_file)).astype(np.float32)  # BGR, uint8
    ...
    def get_road_plane(self, idx):
        ...
        print('plane_file_info : ',plane_file)
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if count_inside_pts:
                    ...
                    target_gt_points = self.get_target_gt_lidar(sample_idx) # SP fusion                    
                    calib = self.get_calib(sample_idx)
    ...
    def __getitem__(self, index):
        ...
        if "points" in get_item_list:
            ...
            # SP fusion
            if self.dataset_cfg.FOV_POINTS_ONLY:
                target_gt_pts_rect = calib.lidar_to_rect(target_gt_points[:, 0:3])
                target_gt_fov_flag = self.get_fov_flag(target_gt_pts_rect, img_shape, calib)
                target_gt_points = target_gt_points[target_gt_fov_flag]
            input_dict['target_gt_points'] = target_gt_points    
