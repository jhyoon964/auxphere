...
class DataProcessor(object):
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        ...
        if data_dict.get('points', None) is not None:
            ...
            # SP fusion
            gt_mask = common_utils.mask_points_by_range(data_dict['target_gt_points'], self.point_cloud_range)
            data_dict['target_gt_points'] = data_dict['target_gt_points'][gt_mask]
    ...
    def shuffle_points(self, data_dict=None, config=None):
        ...
        if config.SHUFFLE_ENABLED[self.mode]:
            ...
            # SP fusion
            target_gt_points = data_dict['target_gt_points']
            target_gt_shuffle_idx = np.random.permutation(target_gt_points.shape[0])
            target_gt_points = target_gt_points[target_gt_shuffle_idx]
            data_dict['target_gt_points'] = target_gt_points
        return data_dict
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        ...
        points = data_dict['points']
        target_gt_points = data_dict['target_gt_points'] # SP fusion
        voxel_output = self.voxel_generator.generate(points)
        voxels, coordinates, num_points = voxel_output

        
        target_gt_voxel_output = self.voxel_generator.generate(target_gt_points) # SP fusion
        target_gt_voxels, target_gt_coordinates, target_gt_num_points = target_gt_voxel_output # SP fusion

        if not data_dict['use_lead_xyz']:
            voxels = voxels[..., 3:]  # remove xyz in voxels(N, 3)
            # target_gt_voxels = target_gt_voxels[..., 3:]  # SP fusion

        data_dict['voxels'] = voxels
        data_dict['voxel_coords'] = coordinates
        data_dict['voxel_num_points'] = num_points
        # SP fusion
        data_dict['target_gt_voxels'] = target_gt_voxels
        data_dict['target_gt_voxel_coords'] = target_gt_coordinates
        data_dict['target_gt_voxel_num_points'] = target_gt_num_points
        return data_dict
    ...
    def sample_points(self, data_dict=None, config=None):
        ...
        points = data_dict['points']
        target_gt_points = data_dict['traget_gt_points'] # SP fusion
        ...
        data_dict['target_gt_points'] = target_gt_points[choice] # SP fusion
        return data_dict
    ...
    def imrescale(self, data_dict=None, config=None):
        ...
        gt_img = cv2.resize(gt_img, new_size, interpolation=cv2.INTER_LINEAR) # addition
        w_scale, h_scale = new_size[0] / w, new_size[1] / h
        data_dict['image'] = img
        data_dict['gt_image'] = gt_img # addition
