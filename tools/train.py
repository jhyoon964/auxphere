...
def main():
    ...
    ckpt_dir = output_dir / 'ckpt' / 'rain_x1_rain_re_experiment'  #/ 'ablation' / 'row4_rain'   #/ 'lidar_fusion_x4_rain_011'  
    ...
    if dist_train:
        model = nn.parallel.DistributedDataParallel(model, device_ids=[cfg.LOCAL_RANK % torch.cuda.device_count()], find_unused_parameters=True)#False
    logger.info(model)
